...
public class Game_Dungeon implements Serializable {
	...
	public ArrayList<int[]>[] doors_room;

	public int[] stairs_up;

	public ArrayList<ArrayList<Integer>> room_linked;

	public boolean[] room_visited;

	public int nbr_lock = 0;

	@SuppressWarnings("unchecked")
	public Game_Dungeon(int i, int j, int k, int l) {
		max_x = i;
		max_y = j;
		...
		doors_room = new ArrayList[rooms_max + 1];

		room_linked = new ArrayList<ArrayList<Integer>>();

		room_visited = new boolean[rooms_max];
		...
		for (int l2 = 1; l2 < doors_room.length - 1; ++l2) {
			if (doors_room[l2] != null)
				for (int l3 = 0; l3 < doors_room[l2].size(); ++l3) {
					...
					for (int l4 = 0; l4 < 4; ++l4) {
						...
						int x1 = doors_room[l2].get(l3)[0];
						int y1 = doors_room[l2].get(l3)[1];

						int r = new Random().nextInt(100);

						if (r > 70)
							if (room_id[x1 + d1][y1 + d2] == room_id[x1][y1]
									&& map[x1 + d1][y1 + d2] == tileDirtFloor) {
								map[x1 + d1][y1 + d2] = 28;
							}
					}
				}
		...
		for (int q = 0; q < max_x; ++q) {
			for (int s = 0; s < max_y; ++s) {
				...
				if (map[q][s] == tileCouloir) {
					map[q][s] = tileDirtFloor;
				}
	...
	private void force_make_room(int sx, int sy, int x, int y) {
		...
		for (int r = 0; r < sx; ++r) {
			for (int t = 0; t < sy; ++t) {
				...
				} else if (nbr_room == this.rooms_max) {
					if (r == sx / 2 && t == sy / 2) {
		...
		if (wall != null) {
			...
			if (d1 != null && d2 != null) {
				...
				if (list != null && list.size() > 0) {
					...
					if (doors_room[rl] == null)
						doors_room[rl] = new ArrayList<int[]>();
					doors_room[rl].add(d1);

					int rl2 = room_id[e.x][e.y];

					if (doors_room[rl2] == null)
						doors_room[rl2] = new ArrayList<int[]>();

					doors_room[rl2].add(d2);
